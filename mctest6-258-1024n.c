...
/*
 * Compilación:
 * gcc -O3 -march=native -static mctest6-258-1024n.c -o mctest6-258-1024n.exe -lgmp
 * Uso:
 * ./mctest6-258-1024n <n>
 * Ejemplo para n=0 (k=258):  ./mctest6-258-1024n 0
 * Ejemplo para n=1 (k=1282):  ./mctest5-258-1024n 1
 */
...
int main(int argc, char *argv[]) {
    ...
    unsigned long k = 258 + 1024 * n_input;
    ...
    unsigned long pasos_k = 2 * k + 21;
    unsigned long pasos_k1 = 2 * k + 22;

    // Iterar Mk por (2k + 21) pasos
    for (unsigned long i = 0; i < pasos_k; i++) {
        paso_collatz_estandar(estado_k);
    }

    // Iterar Mk+1 por (2k + 22) pasos
    for (unsigned long i = 0; i < pasos_k1; i++) {
        paso_collatz_estandar(estado_k1);
    }

    // Calcular el valor teórico mediante la fórmula cerrada: N(k) = (3^(k+7) + 55069) / 16384
    mpz_t N_teorico, num, denom;
    mpz_inits(N_teorico, num, denom, NULL);

    mpz_ui_pow_ui(num, 3, k + 7);
    mpz_add_ui(num, num, 55069);
    mpz_set_ui(denom, 16384);
