...
/*
 * Compilación:
 * gcc -O3 -march=native -static mctest3-206-256n.c -o mctest3-206-256n.exe -lgmp
 * Uso:
 * ./mctest3-206-256n <n>
 * Ejemplo para n=0 (k=206):  ./mctest3-206-256n 0
 * Ejemplo para n=1 (k=462):  ./mctest3-206-256n 1
 */
...
int main(int argc, char *argv[]) {
    ...
    unsigned long k = 206 + 256 * n_input;
    ...
    unsigned long pasos_k = 2 * k + 12;
    unsigned long pasos_k1 = 2 * k + 13;

    // Iterar Mk por (2k + 12) pasos
    for (unsigned long i = 0; i < pasos_k; i++) {
        paso_collatz_estandar(estado_k);
    }

    // Iterar Mk+1 por (2k + 13) pasos
    ...
    mpz_add_ui(num, num, 439);
    mpz_set_ui(denom, 256);
