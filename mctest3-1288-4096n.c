...
/*
 * Compilación:
 * gcc -O3 -march=native -static mctest3-1288-4096n.c -o mctest3-1288-4096n.exe -lgmp
 * Uso:
 * ./mctest3-1288-4096n <n>
 * Ejemplo para n=0 (k=1288):  ./mctest3-1288-4096n 0
 * Ejemplo para n=1 (k=5385):  ./mctest3-1288-4096n 1
 */
...
int main(int argc, char *argv[]) {
    ...
    unsigned long k = 1288 + 4096 * n_input;
    ...
    unsigned long pasos_k = 2 * k + 17;
    unsigned long pasos_k1 = 2 * k + 18;

    // Iterar Mk por (2k + 17) pasos
    for (unsigned long i = 0; i < pasos_k; i++) {
        paso_collatz_estandar(estado_k);
    }

    // Iterar Mk+1 por (2k + 18) pasos
    for (unsigned long i = 0; i < pasos_k1; i++) {
        paso_collatz_estandar(estado_k1);
    }

    // Calcular el valor teórico mediante la fórmula cerrada: N(k) = (3^(k+4) + 8207) / 8192
    mpz_t N_teorico, num, denom;
    mpz_inits(N_teorico, num, denom, NULL);

    mpz_ui_pow_ui(num, 3, k + 4);
    mpz_add_ui(num, num, 8207);
    mpz_set_ui(denom, 8192);
